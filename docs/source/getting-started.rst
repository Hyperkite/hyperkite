...
.. figure:: _static/black_box_function.png
    :scale: 80
...
Now, we request a new set of hyperparameters we can create a new trial using ``trial = hyperkite.new_trial(study_key)``, which contains a dictionary of all hyperparameter values in ``trial.values``. After trying out these hyperparameter values we can report back the validation loss using ``trial.report_loss(loss)``.
